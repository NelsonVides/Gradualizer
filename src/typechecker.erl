...
expect_binary_type(ElemTy = {type, _, binary, _}, _) ->
    {elem_ty, ElemTy, constraints:empty()};
expect_binary_type(Union = {type, _, union, UnionTys}, TEnv) ->
    {Tys, Cs} = expect_binary_union(UnionTys, [], constraints:empty(), TEnv),
    case Tys of
        [] ->
            {type_error, Union};
        [Ty] ->
...
expect_binary_union([Ty|Tys], AccTy, AccCs, TEnv) ->
    case expect_binary_type(normalize(Ty, TEnv), TEnv) of
        {type_error, _} ->
            expect_binary_union(Tys, AccTy, AccCs, TEnv);
        any ->
...
expect_binary_union([], AccTy, AccCs, _TEnv) ->
    {AccTy, AccCs}.
...
type_check_comprehension_in(Env, ResTy, OrigExpr, bc, Expr, _P, []) ->
    case expect_binary_type(ResTy, Env#env.tenv) of
        any ->
            {_Ty, _VB, Cs} = type_check_expr(Env, Expr),
            {#{}, Cs};
        {elem_ty, ElemTy, Cs1} ->
